...
{  
  ...
  public void processTurn(CombatMove playerMove)
  {
    ...
    for(CombatMove move : movesBySpeed)
    {
      ...
      if(player.getStatusCounter(0) > 0)
      {
        ...
        player.decrementStatusCounter(0);
      }
      
      if(player.getStatusCounter(1) > 0)
      {
        int roll = rand.nextInt(100);
        if(roll < 34)
        {
        ...
        player.decrementStatusCounter(1);
      ...
      if(player.getStatusCounter(0) <= 0)
      {
        // Remove lingering status effects
      }
      
      if(player.getStatusCounter(1) <= 1)
      {
        // Remove lingering status effects
      }
      
      if(player.getStatusCounter(2) <= 2)
      {
        // Remove lingering status effects
