static class DEBUG {
  
  public static final boolean DEV_MODE = false;                          // Game dev mode
  
  public static LevelName DEBUG_START_LEVEL = LevelName.MASTER_BEDROOM; // Set starting level
  
  public static final boolean INTRO_ON = false;                         // Turn on intro screen for game
  public static final boolean MUSIC_ON = true;                          // Turn on music
  
  public static final boolean MOVE_LOGGING = false;                     // Turn on logging for movement
  public static final boolean POSITION_LOGGING = false;
  public static final boolean EVENT_LOGGING = false;                    // Turn on logging in events
  public static final boolean COMPUTATION_LOGGING = false;              // Turn on logging in ComputationEngine
  public static final boolean STATE_LOGGING = false;                    // Turn on logging in StateEngine
  public static final boolean DISPLAY_LOGGING = false;                  // Turn on logging in DisplayEngine
  public static final boolean FILE_LOGGING = false;                     // Turn on file input logging
  public static final boolean DIALOG_LOGGING = false;                    // Turn on dialog logging
  public static final boolean COLOR_LOGGING = false;                    // Turn on pixel color check logging
  
}